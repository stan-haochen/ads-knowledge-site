...
42 组测试已通过，覆盖所有默认实验、随机序列上的 AVL/Splay/红黑/B+ 不变量、排序与最近点对参考对照、DP 最优值、负环传播、近似保证、外排守恒等。浏览器检查覆盖 15 章、32 个实验、播放器、错误输入、参考页以及桌面 1440×1080 / 手机 390×844 布局，无页面 JavaScript 异常。
...
入口 `#btrees/redblack-lab`，第二课各标签与学习路径均有入口。采用经典对称、自底向上的插入/删除修复；不混用左倾红黑树规则。14 个小型案例覆盖插入 Case 1–3、删除 Case 1–4（含课件 2.1 / 2.2）、红子吸收、向上级联、双孩子删除与空树边界。每例提供判断/执行阶段、节点移动与变色动画、稳定对象编号、黑 NIL 与虚拟额外黑标记、所有 NIL 路径的黑数，以及合法状态的 2–3–4 树折叠图。

镜像重放对键取反，并把双孩子删除的后继选择改为前驱，保证执行过程真正左右对称。预测模式在判定帧自动暂停，支持判断反馈；SVG 一页记忆图和 Markdown 讲义可直接下载。可切换完整 NIL、原始字号；图形布局和快照工作不纳入算法时间。

`tests/redblack-lab.test.mjs` 检查 5,525 次混合更新、所有中间帧的有效黑路径守恒、合法完成态性质、反向父指针、稳定身份和插入/删除旋转上限。`node tests/redblack-lab-browser.mjs` 检查全部案例、镜像、预测、导出、手机布局和路由清理，支持 `ADS_BASE_URL`。

## Top-down 与 Bottom-up 插入对比

入口 `#btrees/rb-compare`，采用共同初始树，并排展示经典 Bottom-up 与 Weiss 风格对称 Top-down 的变色、分裂和旋转。17 节点主例插入 110：前者无需修复，后者在挂入新键前双旋，最终树形不同。可调极端族含 7/19/43/91 个节点，展示 0 次修复与 Θ(log n) 次预先分裂的差异，附合法性和精确计数证明；另含重复键、相同分裂但发生时机不同的案例。

支持各侧独立回放、同步事件回放、按挂入时刻对齐、路径折叠/完整树、SVG 导出与自定义序列。统计不包含快照和绘图成本，也不把事件数当 CPU 时间。`tests/rb-compare.test.mjs` 核对随机序列、所有中间黑路径、完整红黑性质和极端族计数；`node tests/rb-compare-browser.mjs` 检查五类案例、四种规模、回放、导出、导航与移动布局。全站现有 47 项算法/内容测试。


## 课件 Case 对齐与状态转移

红黑树专题依据 `ADS02BTree_Stu.ppt` 第 4–7 页与 CLRS 第三版 §13.3–13.4（纸质页 319–321、327–330）统一 Case 名称、共同前提、镜像条件、伪代码与章节概览。课件 Case 2.1 / 2.2 是删除 Case 2 按执行前父色的细分；动画在条件成立时固定该标签，不用变色后的颜色重新判别。补充了独立的 Case 2.1 案例。

插入与删除状态图使用 SVG，标注条件、操作、返回入口和终止路径，支持跟随动画高亮、Case 点击/键盘跳转、无图转移列表与 SVG 下载。Markdown 讲义包含映射表和全部转移。`node tests/redblack-cases-browser.mjs` 核对九个图中 Case 入口、预测遮挡、跟随模式、两张图导出和手机横向画布；红黑树原浏览器测试覆盖 14 案例、76 个关键状态。原课件、教材及提取的图文只用于本地核对，不上传。
