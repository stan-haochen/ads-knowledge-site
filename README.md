...
入口 `#btrees/redblack-lab`，第二课各标签与学习路径均有入口。采用经典对称、自底向上的插入/删除修复；不混用左倾红黑树规则。13 个小型案例覆盖插入 I1–I3、删除 D1–D4、红子吸收、向上级联、双孩子删除与空树边界。每例提供判断/执行阶段、节点移动与变色动画、稳定对象编号、黑 NIL 与虚拟额外黑标记、所有 NIL 路径的黑数，以及合法状态的 2–3–4 树折叠图。
...
支持各侧独立回放、同步事件回放、按挂入时刻对齐、路径折叠/完整树、SVG 导出与自定义序列。统计不包含快照和绘图成本，也不把事件数当 CPU 时间。`tests/rb-compare.test.mjs` 核对随机序列、所有中间黑路径、完整红黑性质和极端族计数；`node tests/rb-compare-browser.mjs` 检查五类案例、四种规模、回放、导出、导航与移动布局。全站现有 46 项算法/内容测试。
