...
import {caseLabel,transitionSVG,transitionModels} from '../dist/redblack-case-map.js';
function inorder(n){return n.nil?[]:[...inorder(n.left),n.key,...inorder(n.right)]}
test('course cases match textbook guards, mirror labels, and genuine control flow',()=>{
 const seen=new Set(),find=(n,id)=>n.id===id?n:n.nil?null:find(n.left,id)||find(n.right,id);
 for(const p of rbPresets)for(const mirror of [false,true]){const fs=rbLabTrace(parseRBKeys(p.initial).map(k=>mirror?-k:k),parseRBOps(p.ops).map(o=>({...o,key:mirror?-o.key:o.key})),{predecessor:mirror});const decisions=fs.filter(f=>f.phase==='decision');for(const [i,f]of decisions.entries()){const col=r=>find(f.root,f.roles[r])?.color,id=f.courseCase;seen.add(id);assert.ok(f.title.startsWith(caseLabel(id)));if(f.caseId==='D2'){assert.equal(id,col('P')==='R'?'D21':'D22');assert.equal(col('S'),'B');assert.equal(col('N'),'B');assert.equal(col('F'),'B');if(id==='D21')assert.ok(['D-absorb','D-root'].includes(decisions[i+1].caseId))}if(f.caseId==='I2')assert.equal(decisions[i+1].caseId,'I3');if(f.caseId==='D3')assert.equal(decisions[i+1].caseId,'D4');if(f.caseId==='D1')assert.ok(['D21','D3','D4'].includes(decisions[i+1].courseCase))}}
 for(const id of ['I1','I2','I3','D1','D21','D22','D3','D4'])assert.ok(seen.has(id),id);
 for(const [kind,m]of Object.entries(transitionModels)){const ids=new Set(m.nodes.map(n=>n[0]));for(const [a,b]of m.edges){assert.ok(ids.has(a));assert.ok(ids.has(b))}assert.ok(transitionSVG(kind).includes('role="img"'))}assert.ok(rbMemoryText().includes('Case 2.1'));assert.ok(rbMemoryText().includes('状态转移'));
});
