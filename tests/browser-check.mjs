...
let checked=0;
for(const c of chapters){await page.goto('http://127.0.0.1:8765/#'+c.id);await page.locator('#demo').waitFor();for(const d of c.demos){await page.locator('#demo').selectOption(d);assert.equal(await page.locator('#error').innerText(),'');assert.ok((await page.locator('#stage').innerHTML()).length>30);const max=await page.locator('#seek').getAttribute('max');await page.locator('#seek').fill(max);assert.match(await page.locator('#step-number').innerText(),new RegExp('/ '+(+max+1)+'$'));await page.locator('#prev').click();await page.locator('#next').click();await page.locator('#reset').click();assert.ok(await page.locator('#prev').isDisabled());checked++}for(const t of ['proof','extend','practice']){await page.locator('#tab-'+t).click();assert.ok((await page.locator('#panel').innerText()).length>80)}await page.locator('details summary').first().click();assert.ok(await page.locator('details').first().getAttribute('open')!==null)}
await page.goto('http://127.0.0.1:8765/#avl');await page.locator('#input').fill('30,20,10,25,28,27');await page.locator('#run').click();await page.locator('#seek').fill(await page.locator('#seek').getAttribute('max'));await page.screenshot({path:'qa-output/desktop-avl.png'});
...
await page.goto('http://127.0.0.1:8765/#sources');assert.match(await page.locator('h1').innerText(),/课件与参考资料/);assert.equal(await page.locator('tbody tr').count(),15);
await page.goto('http://127.0.0.1:8765/#overview');assert.match(await page.locator('h1').innerText(),/高级数据结构/);
await page.setViewportSize({width:390,height:844});
for(const id of ['avl','btrees','dp','backtracking','sources','overview']){await page.goto('http://127.0.0.1:8765/#'+id);const overflow=await page.evaluate(()=>document.documentElement.scrollWidth>window.innerWidth+1);assert.equal(overflow,false,'mobile overflow '+id)}
await page.goto('http://127.0.0.1:8765/#avl');await page.locator('#seek').fill(await page.locator('#seek').getAttribute('max'));await page.screenshot({path:'qa-output/mobile-avl.png',fullPage:true});await page.locator('#menu').click();assert.ok(await page.locator('body').evaluate(el=>el.classList.contains('menu-open')));await page.locator('a[data-chapter="dp"]').click();assert.equal(await page.locator('body').evaluate(el=>el.classList.contains('menu-open')),false);
...
await toolsPage.goto('http://127.0.0.1:8765');
